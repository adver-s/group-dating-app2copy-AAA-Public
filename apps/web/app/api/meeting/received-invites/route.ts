...
export async function GET(req: NextRequest) {
  try {
    ...
    // このチームが受け取ったマッチングフローを取得（重複を除外）
    const receivedInvites = await executeQuery(`
      SELECT 
      ...
      WHERE tmf.to_group_id = ?
      AND tmf.id = (
        SELECT id
        FROM team_matching_flows tmf2
        WHERE tmf2.to_group_id = ?
        AND tmf2.from_group_id = tmf.from_group_id
    ...
    console.log('受け取ったお誘いのデータ:', receivedInvites);
