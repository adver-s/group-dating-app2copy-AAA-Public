...
本リポジトリは、個人開発プロジェクトの開発途中の状態を含んでおり、
機能追加を優先して進めてきた結果、ディレクトリ構成や責務分離が一部整理しきれていない箇所があります。

現在、設計の見直しと保守性向上を目的として、
新しいリポジトリ構造への段階的な移行を進めています。

新構造の設計方針およびディレクトリ構成のみをまとめた
設計用リポジトリ（コード未実装）は、同一GitHub組織内の  
`gathers_blueprint` リポジトリにて公開しています。

本リポジトリでは「現行実装」、
`gathers_blueprint` では「今後の設計・構造方針」をご確認いただければ幸いです。
